...
values can be binar ized in some ways. We also mentioned that the \ac{MMHC} algorithm, which 
...
as glial markers referring a review article\cite{zhang2015molecular}. Those glial markers are subcrribed to various GO terms in total, and there 
...
developped dropout imputation algorithms. To clarify our point, here we examine how dropouts explain the data features.
...
transformation on the mean expression values (See Appendix for details). The logistic-transformed mean values 
...
(\figurename{ 4B}). We named the calibration curve the \ac{LM}. To benchmark the performance of the LM, 
we made a Poisson regression model and a \ac{NB} regression model, as those regression models are 
regular choices for modeling dropout events\cite{choi2020bayesian}.


using the PBMC3k dataset and new group of datasets obtained by Mereu and the colleagues\cite{mereu2020benchmarking} (hereby 
we called the group of datasets Mereu2020).
...
\begin{figure}[htb]
  ...
  \legend{
    ...
    The $+$ refers to non-zero expression values and the $-$ to zeros.
    \textbf{B}: The logistic model of DOR. 
...
Mereu2020 includes 15 superfamilies where the same sample components were measured across different protocols 
(e.g., different platforms, or different sequencing depth) in order to benchmark scRNA-seq protocols\cite{mereu2020benchmarking}.
