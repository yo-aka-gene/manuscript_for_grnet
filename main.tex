...
importance scores, SHAP scores can be an alternative metric to visualize the correspondence between the features 
and the classifications. Even though SHAP scores provided more intuitive and precise explanations (\figurename{ 2D and S3A-I}), 
...
row of the heatmap, the $d^*$ values were all zero for all clusters from the point of view of cluster 8, implying that 
the clusters 0$\sim$7 are indistinguishable from the cluster 8. Given that the cluster 8 showed significantly different 
expression patterns of the top 5 marker genes (\figurename{ 2F}), the results of $d^*$ values were against intuition. Additionally, the 
top 30 upregulated GO terms of each cluster indicate that cluster 8 can be annotated as megakaryocyte
...
also visualized the feature importance values implemented in LightGBM. SHAP scores were calculated and visualized 
with a Python package, Shap.
