...
indicator of enriched gene expressions. Contrary, some experts have been opposing to the thoughts on exploiting the dropouts and have 
developped dropout imputation algorithms. To clarify our point, here we examined how dropouts explains the data 
...
(\figurename{ 4A}). We named the calibration curve the \ac{LM}. To benchmark the performance of the LM, 
...
\begin{figure}[htb]
  \centering
  \includegraphics[scale=0.7]{./figs/exported/logisticmodel.png}
  \caption{The top 30 upregulated GO terms of the clusters in PBMC3k}
  \legend{
    \textbf{A}: The logistic model of DOR. 
    \textbf{B}: The performance comparison with the Poisson regression model (Poisson) and 
